...
class EffectivePermissionResolverTests {
    ...
    fun `user with one organisation gets role and direct allow permissions`() {
        val queries = FakePermissionQueries(
            ...
            directPermissions = mapOf(
                membershipId to listOf(PermissionEffectAssignment("iam.user.invite", PermissionEffect.ALLOW)),
            ),
        )
    ...
    fun `same user with two organisations receives different active organisation permissions`() {
        ...
        assertEquals(setOf("logistics.shipment.approve"), resolver.effectivePermissions(membershipId))
        assertEquals(setOf("accounting.journal.post"), resolver.effectivePermissions(secondMembershipId))
    }
    ...
    fun `direct deny overrides role allow`() {
        val resolver = resolver(
            FakePermissionQueries(
                ...
                rolePermissions = mapOf(membershipId to setOf("logistics.shipment.approve")),
                directPermissions = mapOf(
                    membershipId to listOf(PermissionEffectAssignment("logistics.shipment.approve", PermissionEffect.DENY)),
                ),
            ),
        )
    ...
    fun `permission cache invalidation reloads changed role and direct permissions`() {
        ...
        queries.directPermissions = mapOf(
            membershipId to listOf(PermissionEffectAssignment("iam.user.invite", PermissionEffect.ALLOW)),
        )
    ...
    private fun resolver(queries: FakePermissionQueries): EffectivePermissionResolver {
        return EffectivePermissionResolver(queries, ConcurrentMapCacheManager(EffectivePermissionResolver.CACHE_NAME))
    }
}
...
private class FakePermissionQueries(
    ...
    override fun membershipStatus(membershipId: UUID): MembershipStatus? = membershipStatuses[membershipId]

    override fun rolePermissionCodes(membershipId: UUID): Set<String> = rolePermissions[membershipId].orEmpty()

    override fun directPermissionEffects(membershipId: UUID): List<PermissionEffectAssignment> {
        return directPermissions[membershipId].orEmpty()
    }
}
