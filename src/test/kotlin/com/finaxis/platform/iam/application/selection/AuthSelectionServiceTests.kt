...
class AuthSelectionServiceTests {
    ...
    fun `select organisation returns signed context for active membership`() {
        ...
        val lookup = FakeMembershipLookup(
            userId = userId,
            membership = MembershipSelection(membershipId, userId, organisationId, MembershipStatus.ACTIVE),
        )
    ...
    fun `select organisation auto selects the only assigned branch`() {
        ...
        assertEquals(false, response.requiresBranchSelection)
        assertEquals(ActiveOrganisationContext(userId, organisationId, membershipId, branchId), contextService.verify(response.contextToken))
        assertEquals(ActiveOrganisationContext(userId, organisationId, membershipId, branchId), response.context)
    }
    ...
    fun `select organisation asks client to select branch when multiple branches are assigned`() {
        ...
        assertEquals(ActiveOrganisationContext(userId, organisationId, membershipId, branchId = null), contextService.verify(response.contextToken))
    }
    ...
    fun `select branch rejects missing organisation context`() {
        val service = AuthSelectionService(FakeMembershipLookup(userId = UUID.randomUUID()), contextService)
    ...
    fun `select branch rejects inactive membership`() {
        ...
        val service = AuthSelectionService(
            FakeMembershipLookup(
                userId = userId,
                membership = MembershipSelection(membershipId, userId, organisationId, MembershipStatus.SUSPENDED),
                branchIds = listOf(branchId),
            ),
            contextService,
        )
        ...
        assertThrows<OrganisationSelectionDeniedException> {
            service.selectBranch("keycloak-subject", branchId, ActiveOrganisationContext(userId, organisationId, membershipId))
        }
    }
    ...
    fun `select organisation rejects suspended membership`() {
        ...
        val service = AuthSelectionService(
            FakeMembershipLookup(
                userId = userId,
                membership = MembershipSelection(UUID.randomUUID(), userId, organisationId, MembershipStatus.SUSPENDED),
            ),
            contextService,
        )
...
private class FakeMembershipLookup(
    ...
    override fun findMembership(userId: UUID, organisationId: UUID): MembershipSelection? = membership
