...
class WebAdapterUnitTests {
    ...
    fun `auth controller returns selected organisation response`() {
        ...
        val service = AuthSelectionService(
            lookup = StaticMembershipLookup(organisationId, membershipId),
            contextService = ActiveOrganisationContextService(
        ...
        val response = controller.selectOrganisation(jwt(), SelectOrganisationRequest(organisationId), MockHttpSession())
    ...
    fun `auth controller stores selected branch in browser session`() {
        ...
        val lookup = StaticMembershipLookup(organisationId, membershipId, branchIds = listOf(branchId))
        val service = AuthSelectionService(
            lookup = lookup,
            contextService = ActiveOrganisationContextService(
                ActiveOrganisationContextProperties("test-secret-with-enough-length"),
        ...
        val response = controller.selectBranch(jwt(), SelectBranchRequest(branchId), session)
    ...
    fun `api exception handler returns forbidden response`() {
        ...
        assertEquals("denied", response.body?.errors?.single()?.message)
    }

    @Test
    fun `api exception handler returns spring security forbidden response`() {
        val response = ApiExceptionHandler().springSecurityForbidden(
            org.springframework.security.access.AccessDeniedException("missing authority"),
            MockHttpServletRequest("GET", "/protected"),
        )

        assertEquals(403, response.statusCode.value())
        assertEquals("forbidden", response.body?.code)
        assertEquals("authorization", response.body?.errors?.single()?.attribute)
    }
    ...
    fun `api exception handler returns validation response`() {
        ...
        val exception = MethodArgumentNotValidException(MethodParameter(dummyBodyMethod(), 0), binding)
        val response = ApiExceptionHandler().validation(exception, MockHttpServletRequest("POST", "/auth/select-organisation"))

        assertEquals(400, response.statusCode.value())
        assertEquals("Validation failed", response.body?.message)
        assertEquals(mapOf("organisationId" to listOf("must not be null")), response.body?.fieldErrors)
    }
    ...
    fun `api exception handler maps method validation response`() {
        ...
        val result = ParameterValidationResult(
        ...
        val response = ApiExceptionHandler().methodValidation(exception, MockHttpServletRequest("GET", "/bad"))
    ...
    fun `api exception handler returns type mismatch response`() {
        val exception = MethodArgumentTypeMismatchException("abc", UUID::class.java, "id", MethodParameter(dummyIdMethod(), 0), null)
        val response = ApiExceptionHandler().typeMismatch(exception, MockHttpServletRequest("GET", "/organisations/abc"))

        assertEquals(400, response.statusCode.value())
        assertEquals("invalid_parameter", response.body?.code)
        assertEquals("id", response.body?.errors?.single()?.attribute)
    }

    @Test
    fun `api exception handler returns type mismatch response for unknown type`() {
        val exception = MethodArgumentTypeMismatchException("abc", null, "id", MethodParameter(dummyIdMethod(), 0), null)
        val response = ApiExceptionHandler().typeMismatch(exception, MockHttpServletRequest("GET", "/organisations/abc"))

        assertEquals(400, response.statusCode.value())
        assertEquals("Invalid value 'abc' for parameter 'id'. Expected type: unknown", response.body?.message)
    }

    @Test
    fun `api exception handler maps missing parameter response`() {
        val response = ApiExceptionHandler().missingParameter(
        ...
        assertEquals("organisationId", response.body?.errors?.single()?.attribute)
    }

    @Test
    fun `api exception handler maps invalid json response`() {
        val response = ApiExceptionHandler().invalidJson(
        ...
        assertEquals("request_body", response.body?.errors?.single()?.attribute)
    }

    @Test
    fun `api exception handler maps missing resource response`() {
        val response = ApiExceptionHandler().noResource(
    ...
    fun `api exception handler maps method not allowed response`() {
        val response = ApiExceptionHandler().methodNotAllowed(
        ...
        assertEquals("http_method", response.body?.errors?.single()?.attribute)
    }
    ...
    fun `api exception handler maps response status exceptions`() {
        val response = ApiExceptionHandler().responseStatus(
    ...
    fun `api exception handler maps illegal argument response`() {
        val response = ApiExceptionHandler().illegalArgument(
        ...
        assertEquals("request", response.body?.errors?.single()?.attribute)
    }
    ...
    fun `api exception handler maps illegal argument without message`() {
        ...
        assertEquals("Invalid argument", response.body?.errors?.single()?.message)
    }
    ...
    fun `api exception handler includes trace id when available`() {
        ...
        val response = ApiExceptionHandler(tracer).unexpected(RuntimeException("boom"), MockHttpServletRequest("GET", "/x"))
    ...
    fun `api exception handler maps constraint violations`() {
        ...
        val response = ApiExceptionHandler().constraintViolation(
    ...
    fun `api exception handler hides unexpected exception details`() {
        val response = ApiExceptionHandler().unexpected(RuntimeException("database password leaked"), MockHttpServletRequest("GET", "/x"))
    ...
    private fun dummyBodyMethod(): Method {
        return WebAdapterUnitTests::class.java.getDeclaredMethod("dummyEndpoint", SelectOrganisationRequest::class.java)
    }

    private fun dummyIdMethod(): Method {
        return WebAdapterUnitTests::class.java.getDeclaredMethod("dummyIdEndpoint", UUID::class.java)
    }
...
private class StaticMembershipLookup(
    ...
    override fun hasAssignedBranch(membershipId: UUID, branchId: UUID): Boolean = branchId in branchIds
}
