...
`PostingTransactionBoundary` in `accounting.application.posting` — the module's one non-domain
...
**`requireFunctionalCurrencyUnchanged` is kept, and it can no longer fire on the posting path.**
`PostingEngine.post` reads the functional currency before `postNew` takes
`pg_advisory_xact_lock_shared`, and re-reads it under the lock. At READ COMMITTED the re-read is
fresh and the check is a real detector. At `SERIALIZABLE` both reads come from one snapshot, so the
comparison becomes a tautology **and the window it guarded is left open**. Four measurements, with
the snapshot pinned by earlier work as it always is in production:
...
(c) is decisive: hoisting the advisory lock above the first read — the obvious repair, and one of
the reviewed designs — does **not** work, because an advisory lock does not participate in MVCC and
so brings neither `EvalPlanQual` nor a `40001` with it. Only (d), a *locking* read of the
organisation row, closes the window. `AccountingTenantLookup.functionalCurrencyOf` is a
cross-module port implemented by `LifecycleAccountingTenantAdapter`, so that repair is a
cross-module port change plus an ADR 0023 lock-ordering amendment, and it is out of scope for an
issue about isolation; **a follow-up issue** owns it. The check and
`PostingErrorCodes.FUNCTIONAL_CURRENCY_CHANGED` are kept with their KDoc rewritten to say this
plainly. They are **not** deleted on the argument that the advisory lock was always the real
guarantee: that argument is false, the existing KDoc on the engine already explains why it never
was, and deleting a detector that fires today at READ COMMITTED would make the regression
invisible as well as open.
...
The retry is therefore load-bearing for the *error message*, not only for throughput. This follows
...
**Issue #52's accounting controllers must call `PostingTransactionBoundary`, with the idempotency
